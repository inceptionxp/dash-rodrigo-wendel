...
FRAMEWORKS: [
  ...
  {
    id: 'motor-de-tracao', n: 'F7', nome: 'Motor de Tração', classe: 'framework de etapa · Negócio (básico na formação = 1 motor · arsenal completo = mentoria)',
    status_nome: '✓ nomeado pela consultora · 04/07 · corrigido em 21/08 (devolutiva outputs/devolutivas/motores-de-tracao.md)',
    o_que_e: 'O jeito que o consultório atrai paciente novo. O básico da formação ensina UM motor só — indicação + posicionamento pra base atual (o mesmo que já responde por 69% da receita dele) — antes de qualquer conversa sobre anúncio. O arsenal completo (tráfego pago, conteúdo, Google, parcerias) fica pra mentoria.',
    como_funciona: [
      'BÁSICO (formação, Encontros 39-40): scripts de recepção pra converter lead em avaliação (E39) + posicionamento e indicação pra base atual e rede pessoal, o canal mais barato de quem está começando (E40). Um motor, justificado pelos números reais dele: indicação é 15% do volume da clínica e 69% da receita, com conversão lead→avaliação de 67,2% — 11× a conversão do tráfego pago (Meta: 33% do volume, 12% da receita, 5,9% de conversão; em fevereiro, 55 leads de Meta geraram 0 avaliações).',
      'ARSENAL COMPLETO (mentoria): catálogo de motores (tráfego pago · conteúdo orgânico/redes · Google e busca · rede de encaminhadores/colegas · autoridade local/palestras · convênios) + matriz motor × perfil de aluno, pra quem já validou o básico e quer diversificar ou escalar além da indicação.',
      'É a lógica clínica da Abordagem MODAL aplicada ao negócio: assim como não se trata todo paciente com uma única técnica, não se capta todo paciente com um único funil — mas o aluno começa por UM motor que funciona, não por um arsenal.',
    ],
    quando_usar: 'Na formação, o básico: scripts de recepção (Encontro 39) + posicionamento e indicação pra base atual (Encontro 40). Na mentoria, o arsenal completo: tráfego pago, conteúdo multi-plataforma, Google, parcerias — com o mix desenhado por aluno via matriz motor × perfil.',
    por_que_funciona: 'Ancorado no dado real dele, não em preferência de mercado: indicação converte 11× mais que tráfego pago no funil da própria clínica. Simetria com o método clínico: a mesma filosofia de individualização do tratamento aplicada à captação — o que torna o framework inconfundível com "curso de marketing pra dentista".',
    analogia: '"Pra quem só tem martelo, tudo é prego" também vale pra captação — mas o básico da formação nem chega a abrir a caixa de ferramentas: entrega só a que já funciona pra ele.',
    temos: ['A tese e o diferencial (call + direcionamento da consultora em 04/07)', 'O caso real dele: Google + Instagram + indicação + paciente vindo do ChatGPT', 'Os números que ancoram a escolha do motor básico (planilha jan-jun/26)', 'Catálogo de motores + matriz motor × perfil de aluno (devolutiva `outputs/devolutivas/motores-de-tracao.md`, 21/08 — aguarda aval do Rodrigo)'],
    falta: ['Aval do Rodrigo sobre o catálogo de motores e a matriz motor × perfil (itens DERIVADOS da devolutiva)'],
    vira_conteudo: ['Encontro 39 (scripts de recepção) + Encontro 40 (posicionamento e indicação — o básico)', 'Mentoria: módulo de tração completo com a matriz motor × perfil', 'Copy: "o motor que já converte 11× mais é o que a formação ensina primeiro"'],
...
INSTRUMENTOS: [
  ...
  { nome: 'Mapa de Motores de Tração (matriz motor × perfil)', formato: 'Matriz/ferramenta de escolha', etapa: 'Negócio · F7 (instrumento de mentoria — o básico da formação usa 1 motor só)', uso: 'O aluno, já na mentoria, cruza o arsenal completo de motores com o próprio perfil (base própria, rede profissional, capital, tempo) e monta o mix de tração além da indicação', status: 'catálogo + matriz entregues em outputs/devolutivas/motores-de-tracao.md (21/08) — aguarda aval do Rodrigo' },
